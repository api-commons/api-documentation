...
function mailto(s: Service, ctx: string): string {
  const body = `${s.body(ctx)}\n\n— sent from ${APP} (documentation.apicommons.org)`;
  ...
  modal.innerHTML = `
    <div class="modal-card engage-card">
      ...
      <div class="engage-body">
        <p class="engage-intro">API Documentation is open and free to run yourself. When you want experts in the loop,
